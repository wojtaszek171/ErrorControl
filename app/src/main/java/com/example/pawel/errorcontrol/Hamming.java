...
public class Hamming extends CodeBase
{
	...
	void fix()
	{
		int n = code.length;                                    // dlugosc zakodowanego slowa danych
		int d = 0;												// ilosc danych
		int nadmiar = 0;										// ilosc nadmiarowych bitow

		// oblicz dlugosc danych
		for (int i = 0; i < n; i++)
		{
		...
		if (maska != 0)					// wykryto błąd
		{
			...
			{
				if (type[numer]==0) 				// koryguj bit danych
					type[numer]=1;
				else if (type[numer]==3) 			// koryguj bit redundantny
					type[numer]=4;
